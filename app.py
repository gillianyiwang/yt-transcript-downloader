...
YOUTUBE_RED = "red-6"  # central place to tweak red tone if you ever want to

GLOBAL_CSS = """
body {
    background: radial-gradient(circle at top, #fee2e2 0, #f9fafb 45%, #e5e7eb 100%);
...
def main_page() -> None:
    ...
    # Outer layout to keep content centered and breathable
    with ui.column().classes("items-center justify-center").style(
        ...
        with ui.row().classes("items-center gap-3").style("margin-bottom: 16px;"):
            ...
            with ui.column().classes("gap-0"):
                ...
                ui.label("Extract, trim, and export YouTube transcripts with ease.").classes(
        ...
        with ui.card().classes("w-full").style(
            ...
            # URL + fetch row
            ui.label("1. Paste a YouTube URL").classes("text-sm font-medium text-gray-800")
            ...
            with ui.row().classes("items-center justify-between").style(
                ...
                with ui.row().classes("items-center gap-2"):
                    ...
                    with progress:
                        progress_label = (
                            ui.label("0%")
                            .classes("text-xs text-black absolute-center font-medium")
                            .bind_text_from(
            ...
            with ui.expansion(
                ...
                with ui.row().classes("items-center").style("gap: 12px; margin-top: 4px;"):
                    # START
            ...
            with ui.row().classes("items-center justify-end").style("margin-top: 10px;"):
            ...
            action_status_label = ui.label("").classes("text-xs text-gray-600").style(
                "margin-top: 4px;"
            )
            ...
            def refresh_counts() -> None:
                ...
                counts_label.text = (
                    f"Words: {words} | Characters: {chars} | Est. size: {format_size(size_bytes)}"
                )
